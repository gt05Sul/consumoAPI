...
function displayUsers(usuarios) {
  ...
  usuarios.forEach((user) => {
    ...
    usuario.innerHTML = `
            <div><img src="${user.avatar}" alt="${user.firstName}" class="img rounded-circle" style="max-width: 150px;"></div>
            <div>
                <h3>${user.firstName} ${user.lastName}</h3>
                <p>Email: ${user.email}</p>
                <p>Telefone: ${user.phone}</p>
                <box-icon onclick="deleteUsuario(${user.id})" name='trash-alt'></box-icon>
            </div>
        `;
